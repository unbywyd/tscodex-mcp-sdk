...
export class McpServer extends EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            if (this.rateLimiter) {
                this.rateLimiterCleanupInterval = setInterval(() => this.rateLimiter.cleanup(), 60000 // Cleanup every minute
                );
            }
    ...
    async loadSession() {
        ...
        try {
            ...
            this.session = validateConfig(sessionData, this.options.auth.sessionSchema);
            if (this.logger) {
    ...
    async filterByAccess() {
        ...
        for (const [name, def] of this.tools.entries()) {
            if (def.access && def.access.length > 0) {
                ...
                const accessChecks = await Promise.all(def.access.map(async (role) => {
                    const guard = this.options.auth.roles[role];
                    if (!guard) {
                        if (this.logger) {
     ...
     */
    async handleUpdateProjectRoot(req, res) {
        try {
     ...
     */
    withTimeout(handler, timeoutMs, errorMessage) {
        return Promise.race([
            handler(),
    ...
    handleError(error, errorContext) {
        ...
        const typeName = errorContext.type === 'tool' ? 'tool'
            : errorContext.type === 'resource' ? 'resource'
                : 'prompt';
        return new Error(`An error occurred while executing ${typeName} "${errorContext.name}": ${errorMessage}`);
    }
    /**
    ...
    registerMcpHandlers() {
        ...
        this.mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                const result = await this.withTimeout(() => toolDef.handler(validatedParams, context), this.handlerOptions.toolHandlerTimeout, `Tool "${name}" execution timeout`);
                this.emit('toolCalled', name, validatedParams, result);
                // MCP SDK expects the same type structure we return
                return result; // Type assertion needed due to MCP SDK type limitations
            }
            catch (error) {
                if (this.logger) {
        ...
        this.mcpServer.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            try {
                const result = await this.withTimeout(() => resourceDef.handler(uri, context), this.handlerOptions.resourceHandlerTimeout, `Resource "${uri}" read timeout`);
        ...
        this.mcpServer.setRequestHandler(GetPromptRequestSchema, async (request) => {
            ...
            try {
                const result = await this.withTimeout(() => promptDef.handler(validatedParams, context), this.handlerOptions.promptHandlerTimeout, `Prompt "${name}" execution timeout`);
     ...
     */
    registerDefaultContextResource() {
    ...
    generateResourcePrefix(name, providedId) {
        ...
        if (!/[a-z]/.test(generatedId)) {
            throw new Error(`Cannot generate server ID from name "${name}". ` +
    ...
    addResource(config) {
        ...
        if (!normalizedUri.includes('://')) {
            ...
            if (scheme !== this.resourcePrefix) {
                ...
                if (this.logger) {
                    this.logger.warn(`Resource URI "${config.uri}" uses scheme "${scheme}" instead of server prefix "${this.resourcePrefix}". ` +
    ...
    formatValidationErrors(errors) {
        return errors.map(err => `${err.path || 'root'}: ${err.message || 'Invalid value'}`).join('; ');
    }
    /**
