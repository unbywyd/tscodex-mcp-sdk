 ...
 */
export function safePath(rootPath: string, userPath: string): string {
 ...
 */
export function validateRequestSize(contentLength: string | undefined, maxSize: number = 10 * 1024 * 1024): void {
  if (!contentLength) {
    return;
  }
 ...
 * ```
 */
export function filterSecrets<T extends Record<string, unknown>>(config: T): Record<string, unknown> {
 ...
 * ```
 */
export function filterMcpConfig<T extends Record<string, unknown>>(config: T): Record<string, unknown> {
 ...
 * ```
 */
export function filterMcpPublicConfig<T extends Record<string, unknown>>(config: T): Record<string, unknown> {
  const filtered: Record<string, unknown> = {};
